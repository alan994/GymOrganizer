...
namespace Data.Db.Queries
{
    public static class TermQuery
    {
        ...
        {
            return db.Terms.Where(x => x.TenantId == tenantId && x.Status != Enums.ExistanceStatus.Deleted);
        }

        public static IQueryable<Term> GetAllActiveTerms(this GymOrganizerContext db, Guid tenantId)
        {
            return GetAllTerms(db, tenantId).Where(x => x.Status == Enums.ExistanceStatus.Active);
        }
